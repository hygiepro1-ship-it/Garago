      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-6">
        {garage.coverUrl ? (
          <div className="h-24 sm:h-32 relative overflow-hidden">
            ...
            <img
              src={garage.coverUrl}
              alt={`Photo de couverture de ${garage.name}`}
              draggable={false}
              style={{
                position: "absolute",
                inset: 0,
          ...
          <div className="h-24 sm:h-32" style={{ background: "linear-gradient(90deg, #071428 0%, #0b1f3a 60%, #f97316 100%)" }} />
        )}
        <div className="px-4 sm:px-6 pb-4 sm:pb-6">
          <div className="flex items-start gap-3 sm:gap-4 mb-4">
            <div className="w-16 h-16 sm:w-20 sm:h-20 -mt-8 sm:-mt-10 rounded-xl sm:rounded-2xl border-4 border-white shadow-md flex items-center justify-center text-4xl overflow-hidden relative bg-gray-100 flex-shrink-0">
              {garage.logoUrl ? (
                <>
                  {/* Background layer */}
                  {logoP.color ? (
            ...
            <div className="flex-1">
              <div className="flex items-start justify-between flex-wrap gap-2">
                <div>
                  ...
                  <h1 className="text-xl sm:text-2xl font-black text-gray-900">{garage.name}</h1>
                      ...
                      {svcs.map((s: any) => (
                        <div key={s.id} className="flex items-start justify-between gap-4">
                          ...
                          {(s.priceMin || s.priceMax) && (
                            <span className="text-sm font-semibold whitespace-nowrap flex-shrink-0" style={{ color: "#f97316" }}>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 mb-5">
              ...
              {session && !isOwner && (
                <button
                  ...
                  className="text-sm text-white px-4 py-2 rounded-xl transition-colors font-medium flex-shrink-0" style={{ background: "#f97316" }}
                >
                  {g.leaveReview}
                </button>
              )}
              {!session && (
                <Link href="/connexion" className="text-xs sm:text-sm font-semibold hover:underline flex-shrink-0" style={{ color: "#f97316" }}>
                  {g.signInToReview}
                </Link>
              )}
            </div>
